...
valid_ip4_or_empty(){ [ -z "$1" ] && return 0; echo "$1" | grep -Eq '^([0-9]{1,3}\.){3}[0-9]{1,3}$' || return 1; local o oldifs="$IFS"; IFS='.'; for o in $1; do [ "$o" -ge 0 ] && [ "$o" -le 255 ] || { IFS="$oldifs"; return 1; }; done; IFS="$oldifs"; }
...
install_deps(){
  ...
  if command -v apt-get >/dev/null 2>&1; then apt-get update; DEBIAN_FRONTEND=noninteractive apt-get install -y iptables curl ca-certificates dnsutils; else echo -e "${YELLOW}未检测到 apt-get，请手动安装 iptables、curl。${RESET}"; fi
  echo -e "${GREEN}依赖检查完成。${RESET}"; pause
}

enable_forward(){
  echo 1 > /proc/sys/net/ipv4/ip_forward 2>/dev/null || true
  if [ -d /etc/sysctl.d ]; then echo 'net.ipv4.ip_forward=1' > /etc/sysctl.d/99-ipt-vibe-panel.conf; else touch /etc/sysctl.conf; sed -i '/^\s*#\?\s*net\.ipv4\.ip_forward\s*=/d' /etc/sysctl.conf; echo 'net.ipv4.ip_forward=1' >> /etc/sysctl.conf; fi
...
rule_port_label(){ [ "$1" = "$2" ] && echo "$1" || echo "$1-$2"; }
list_rules(){
  ...
  while IFS='|' read -r id name enabled listen_ip ls le target_host ts te protos resolved; do
    [ -z "$id" ] && continue; [ "$enabled" = "1" ] && st="启用" || st="停用"; lip=${listen_ip:-0.0.0.0}; lport=$(rule_port_label "$ls" "$le"); tport=$(rule_port_label "$ts" "$te")
    printf "%-4s %-8s %-18s %-25s %-9s %-8s %s\n" "$n" "$st" "$lip:$lport" "$target_host:$tport" "$protos" "${resolved:-无}" "$name"; n=$((n+1))
  done < "$RULES_FILE"; pause
}

read_with_default(){ local prompt="$1" def="$2" val; if [ -n "$def" ]; then read -rp "$prompt [$def]: " val; echo "${val:-$def}"; else read -rp "$prompt: " val; echo "$val"; fi; }
collect_rule_fields(){
  local old_name="$1" old_enabled="$2" old_lip="$3" old_ls="$4" old_le="$5" old_host="$6" old_ts="$7" old_te="$8" old_protos="$9"
  R_NAME=$(read_with_default "规则名称" "$old_name"); R_LIP=$(read_with_default "监听 IP，留空表示全部" "$old_lip"); R_LS=$(read_with_default "入口起始端口" "$old_ls"); R_LE=$(read_with_default "入口结束端口，留空等于起始端口" "$old_le"); R_LE=${R_LE:-$R_LS}
  R_HOST=$(read_with_default "目标 IP/域名" "$old_host"); R_TS=$(read_with_default "目标起始端口" "$old_ts"); R_TE=$(read_with_default "目标结束端口，留空等于起始端口" "$old_te"); R_TE=${R_TE:-$R_TS}
  echo "协议：1) TCP  2) UDP  3) TCP+UDP"; local pc dc="3"; [ "$old_protos" = "tcp" ] && dc="1"; [ "$old_protos" = "udp" ] && dc="2"; pc=$(read_with_default "请选择协议" "$dc")
  ...
  local ec; ec=$(read_with_default "是否启用？1启用/0停用" "${old_enabled:-1}"); case "$ec" in 1|y|Y|yes|YES|on|ON) R_ENABLED="1";; 0|n|N|no|NO|off|OFF) R_ENABLED="0";; *) echo -e "${RED}状态无效。${RESET}"; return 1;; esac
  valid_ip4_or_empty "$R_LIP" || { echo -e "${RED}监听 IP 无效。${RESET}"; return 1; }; valid_port "$R_LS" || { echo -e "${RED}入口端口无效。${RESET}"; return 1; }; valid_port "$R_LE" || return 1; valid_host "$R_HOST" || { echo -e "${RED}目标地址无效。${RESET}"; return 1; }; valid_port "$R_TS" || return 1; valid_port "$R_TE" || return 1
  [ "$R_LS" -le "$R_LE" ] && [ "$R_TS" -le "$R_TE" ] || { echo -e "${RED}端口范围反了。${RESET}"; return 1; }; [ $((R_LE-R_LS)) -eq $((R_TE-R_TS)) ] || { echo -e "${RED}入口端口段和目标端口段长度必须一致。${RESET}"; return 1; }
...
select_rule_id(){ ensure_dirs; grep -q '^[^#[:space:]]' "$RULES_FILE" || { echo -e "${YELLOW}暂无规则。${RESET}"; return 1; }; local n=1 choice; while IFS='|' read -r id name enabled listen_ip ls le target_host ts te protos resolved; do [ -z "$id" ] && continue; echo "$n) $name  $target_host:$ts  [$protos]"; n=$((n+1)); done < "$RULES_FILE"; read -rp "请选择序号: " choice; echo "$choice" | grep -Eq '^[0-9]+$' || return 1; n=1; while IFS='|' read -r id name enabled listen_ip ls le target_host ts te protos resolved; do [ -z "$id" ] && continue; if [ "$n" -eq "$choice" ]; then SELECTED_LINE="$id|$name|$enabled|$listen_ip|$ls|$le|$target_host|$ts|$te|$protos|$resolved"; return 0; fi; n=$((n+1)); done < "$RULES_FILE"; return 1; }
delete_rule(){ clear; line; echo -e "${CYAN}删除转发规则${RESET}"; line; select_rule_id || { echo -e "${RED}选择无效。${RESET}"; pause; return; }; local id name; IFS='|' read -r id name _ <<< "$SELECTED_LINE"; read -rp "确认删除 $name ? [y/N]: " yn; case "$yn" in y|Y) awk -F'|' -v id="$id" '$1 != id {print}' "$RULES_FILE" > "$RULES_FILE.tmp" && mv "$RULES_FILE.tmp" "$RULES_FILE"; echo -e "${GREEN}已删除配置。选择“应用规则”后同步到系统。${RESET}";; *) echo "已取消。";; esac; pause; }
...
backup_restore(){ clear; line; echo -e "${CYAN}备份与恢复${RESET}"; line; echo "1) 立即备份当前 iptables"; echo "2) 查看备份文件"; echo "3) 从备份恢复"; echo "0) 返回"; read -rp "请选择: " c; case "$c" in 1) f=$(backup_rules); echo -e "${GREEN}已备份：$f${RESET}";; 2) ls -lh "$BACKUP_DIR" 2>/dev/null || true;; 3) ls -1 "$BACKUP_DIR"/*.rules 2>/dev/null || { echo "暂无备份。"; pause; return; }; read -rp "请输入完整备份路径: " file; if [ -f "$file" ] && command -v iptables-restore >/dev/null 2>&1; then iptables-restore < "$file" && echo -e "${GREEN}已恢复。${RESET}"; save_persistent >/dev/null; else echo -e "${RED}备份文件不存在或 iptables-restore 不可用。${RESET}"; fi;; esac; pause; }
uninstall_panel(){ clear; line; echo -e "${CYAN}卸载面板${RESET}"; line; read -rp "是否同时删除本工具管理的 iptables 规则？[y/N]: " clean; if [ "$clean" = "y" ] || [ "$clean" = "Y" ]; then backup_rules >/dev/null; delete_managed_iptables_rules; save_persistent >/dev/null; fi; rm -f /usr/local/bin/ipt-vibe /usr/local/bin/zf; echo -e "${GREEN}命令已移除。配置目录保留：$STATE_DIR${RESET}"; pause; }
...
main_menu(){ need_root; ensure_dirs; while true; do header; echo "1. 安装/检查依赖"; echo "2. 添加转发规则"; echo "3. 查看转发规则"; echo "4. 修改转发规则"; echo "5. 删除转发规则"; echo "6. 应用规则到 iptables"; echo "7. 查看当前 iptables 规则"; echo "8. 备份与恢复"; echo "9. 卸载面板命令"; echo "0. 退出"; short_line; read -rp "请输入选项: " choice; case "$choice" in 1) install_deps;; 2) add_rule;; 3) list_rules;; 4) edit_rule;; 5) delete_rule;; 6) apply_rules;; 7) show_current_iptables;; 8) backup_restore;; 9) uninstall_panel;; 0) exit 0;; *) echo "无效选项"; sleep 1;; esac; done; }
main_menu
